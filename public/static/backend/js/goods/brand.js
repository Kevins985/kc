...
var appCallback = {
    upload:function(response){
        global.setUploadData('logo',response);
    },
    addSubmit:function (response){
        if(response.status){
            dialog.confirm({
                ...
                cancel_btn:'返回品牌列表',
                ok_callback:function(){
                    window.location.href = App.createUrl('/backend/brand/add');;
                },
                cancel_callback:function(){
                    window.location.href = App.createUrl('/backend/brand/list');
                }
            });
        }
        else{
