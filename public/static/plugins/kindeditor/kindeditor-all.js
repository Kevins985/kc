...
SWFUpload.prototype.initSWFUpload = function (settings) {
	try {
		var swfClient = this;
		swfClient.customSettings = {};	// A container where developers can place their own settings associated with this instance.
		swfClient.settings = settings;
		swfClient.eventQueue = [];
		swfClient.movieName = "SWFUpload_" + SWFUpload.movieCount++;
		swfClient.movieElement = null;
		...
		$('.uploadMultImageBtn').on('change',function(e){
			var ext = ['jpg','png','gif','bmp','jpeg'];
			for(var i = 0; i < this.files.length; i++) {
				var file_url = getObjectURL(this.files[i]);
				var image_cnt = $('.ke-swfupload-body .ke-item').length
				var name = this.files[i].name;
				var value = name.substring(name.lastIndexOf(".") + 1);
				value = value.toLowerCase();
				if ($.inArray(value,ext)=='-1') {
					alert('上传的文件格式必须是('+(ext.join('、'))+')');
					$(this).val("");
					return;
				}
				if(image_cnt<settings.file_upload_limit){
					var up_file = {
						id:hex_md5(name),
						name:name,
						url:file_url,
					...
					if(!$('div[data-id="'+up_file.id+'"]').get(0)){
						settings.file_queued_handler(up_file);
						swfClient.eventQueue.push(up_file);
					}
				}
				else{
					alert('超过允许上传的图片个数'+settings.file_upload_limit);
				}
			}
			e.preventDefault();
		});
...
SWFUpload.prototype.cancelUpload = function (fileID, triggerErrorEvent) {
	...
	if(this.eventQueue.length>0){
		...
		for(var i=0;i<this.eventQueue.length;i++){
			if(this.eventQueue[i].id!=fileID){
				data.push(this.eventQueue[i]);
			}
		}
...
if (typeof(SWFUpload) === "function") {
	...
	SWFUpload.prototype.startUpload = function (fileID) {
		var swfClient = this;
		console.log(swfClient.settings);
		if(swfClient.eventQueue.length==0){
			alert('请添加需要的图片');
		}
		else{
			swfClient.queueSettings.queue_cancelled_flag = false;
			$.each(swfClient.eventQueue,function(i,v){
				var fdata = new FormData($('<form method="post"></form>')[0]);
				fdata.append('user_id',initScriptData.uuid);
				fdata.append('imgFile',v.file);
				fdata.append('type','editor');
				fdata.append('num',i);
				$.ajax({
					type: "post",
					url: swfClient.settings.upload_url,
					async: true,
					data: fdata,
					dataType: "json",
					processData: false,
					...
					success: function(response) {
						if($('div[data-id="'+v.id+'"]').get(0)) {
							if (response.error == 0) {
								v['filestatus'] = -4;
								swfClient.settings.upload_progress_handler(v, 100, 100);
								swfClient.settings.upload_success_handler(v, JSON.stringify(response));
							} else {
								v['filestatus'] = -3;
								swfClient.settings.upload_error_handler(v, response.error, response.message);
							}
						}
					}
				});
