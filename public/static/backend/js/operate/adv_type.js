...
advTypeJs = {
    ...
    initFormScript: function () {
        ...
        $('#advTypeSubmitBtn').on('click',function (){
    ...
    addSubmit:function (response){
        if(response.status){
            dialog.confirm({
                ...
                cancel_btn:'返回广告类型列表',
                ok_callback:function(){
                    window.location.href = App.createUrl('/backend/advType/add');;
                },
                cancel_callback:function(){
                    window.location.href = App.createUrl('/backend/advType/list');
                }
            });
        }
        else{
