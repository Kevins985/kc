var helpCategoryJs = helpCategoryJs || {};
helpCategoryJs = {
    init: function () {
        this.initFormScript();
    ...
    initFormScript: function () {
        ...
        $('#helpCategorySubmitBtn').on('click',function (){
...
var appCallback = {
    addSubmit:function (response){
        if(response.status){
            dialog.confirm({
                ...
                cancel_btn:'返回帮助分类列表',
                ok_callback:function(){
                    window.location.href = App.createUrl('/backend/helpCategory/add');;
                },
                cancel_callback:function(){
                    window.location.href = App.createUrl('/backend/helpCategory/list');
                }
            });
        }
        else{
    ...
    delete: function (url, ids) {
        helpCategoryJs.delete(url,ids[0]);
    },
}
